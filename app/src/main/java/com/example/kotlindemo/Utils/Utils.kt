...
// runtime permission code implemented here
/*
        fun checkAllPermissions(mContext: AppCompatActivity) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                if ((ContextCompat.checkSelfPermission(mContext, Manifest.permission.CAMERA) ==
                    ...
                    (ContextCompat.checkSelfPermission(mContext, Manifest.permission.READ_EXTERNAL_STORAGE) ==
                            PackageManager.PERMISSION_GRANTED)  &&
                    (ContextCompat.checkSelfPermission(mContext, Manifest.permission.READ_CALL_LOG) ==
                            PackageManager.PERMISSION_GRANTED) &&
                    ...
                    requestForPermission(mContext).launch(
                        arrayOf(
                            Manifest.permission.CAMERA,
                            Manifest.permission.WRITE_EXTERNAL_STORAGE,
                            Manifest.permission.READ_EXTERNAL_STORAGE,
        ...
        private fun requestForPermission(mContext: AppCompatActivity): ActivityResultLauncher<Array<String>> {

             return mContext.registerForActivityResult(ActivityResultContracts.RequestMultiplePermissions()) { permission ->
        ...
        fun checkOnlyOnePermission(permission:String,mContext: AppCompatActivity){
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                if ((ContextCompat.checkSelfPermission(mContext, permission) ==
                            PackageManager.PERMISSION_GRANTED)) {
                    Log.e("Permission", "camera permission granted")
                }else if(mContext.shouldShowRequestPermissionRationale(permission)){
                    openSettingDialog(mContext)
                }else{
                    requestForPermission(mContext).launch(
                        arrayOf(
                            permission
        ...
        private fun openSettingDialog(mContext: Context){
